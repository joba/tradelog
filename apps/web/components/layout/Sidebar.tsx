...
import {
  LayoutDashboard, List, BarChart2, PlusCircle, LogOut, Tag, TrendingUp,
} from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-56 bg-terminal-surface border-r border-terminal-border flex flex-col z-40">
      {/* Logo */}
      <div className="px-5 py-5 border-b border-terminal-border">
        <div className="flex items-center gap-2">
          <TrendingUp size={14} className="text-accent" />
          <span className="text-sm font-semibold tracking-[0.2em] text-terminal-bright uppercase">Tradelog</span>
        </div>
        <div className="text-[9px] text-terminal-dim tracking-widest mt-0.5 uppercase">Tradelog</div>
      </div>
      ...
      <div className="px-3 py-4 border-t border-terminal-border space-y-0.5">
        <div className="px-3 py-2">
          <div className="text-[10px] text-terminal-dim truncate">{user?.email}</div>
          <div className="text-xs text-terminal-text truncate">{user?.name || "Trader"}</div>
        </div>
        <button
          onClick={logout}
